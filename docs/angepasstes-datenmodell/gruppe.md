...
faecher[].bezeichnung | String | 0/0..1 | ja | Bezeichnung einens Faches, welches nicht in der Codeliste *Fächerkanon* enthalten ist. Typischerweise ist das Attribut *kennung* dann nicht gesetzt. Sind beide Atrribute vorhanden, so ist die Bezeichnung identisch mit der Bezeichnung des Faches in der Codeliste *Fächerkanon* für den Code in *kennung*.
