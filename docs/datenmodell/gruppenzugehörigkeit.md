...
von | Datum (siehe Abschnitt Datumsformat) | 0..1 | Beginn der Gruppenzugehörigkeit. Dieser Zeitpunkt kann auch in der Zukunft liegen. Ist `von` nicht angegeben, so ist die Gruppenzugehörigkeit für jedes Datum in der Vergangenheit gültig.
bis | Datum (siehe Abschnitt Datumsformat) | 0..1 | Ende der Gruppenzugehörigkeit. Laufzeiten schliessen den ersten und letzten Tag immer mit ein. Ist als `bis` der 31.Oktober 2025 angegeben, so endet die Laufzeit der Gruppenzugehörigkeit am 01.11.2025 um 00:00. Ist `bis` nicht angegeben,so ist die Gruppenzugehörigkeit für jedes Datum in der Zukunft gültig.
