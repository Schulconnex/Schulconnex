...
loeschung.verwaistepersonloeschen | String (Code) | 0..1 | Dieses Attribut gibt an, ob die Daten einer Person automatisch gelöscht werden sollen, wenn der letzte Personenkontext zu dieser Person gelöscht wurde. Ist das Attribut nicht angeben, ist das Verhalten wie bei `Nein`. Referenz auf einen Code der Codeliste *Boolean*.
