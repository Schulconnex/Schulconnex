...
Der Personenkontext umfasst neben der Rolle auch Informationen über die [Organisation](./organisation.md), an der diese Rolle ausgeführt wird.
Eine [Person](./person.md) kann einen oder mehrere Personenkontexte besitzen, indem sie beispielsweise an derselben
...
organisation | [Organisation](./organisation.md) | 1 | Siehe [Organisation](./organisation.md).
rolle | String (Code) | 1 | Rolle der [Person](./person.md) innerhalb der Organisation. Referenz auf einen Code der Codeliste *Rolle*.
...
erreichbarkeiten[].kennung | String (UTF-8) | 1 | Konkrete Angabe der zum Erreichen der [Person](./person.md) oder Organisation notwendigen Information. Derzeit ist nur die Erreichbarkeit über  eine E-Mail- Adresse vorgesehen.
personenstatus | String (Code) | 0..1 | Status, den eine [Person](./person.md) in einer [Organisation](./organisation.md) in Bezug auf eine bestimmte Rolle hat, Referenz auf einen Code der Codeliste *Personenstatus*.
jahrgangsstufe | String (Code) | 0..1 | Jahrgangsstufe, die eine [Person](./person.md) in der [Organisation](./organisation.md) in einer bestimmten Rolle besucht, Referenz auf Code der Codeliste *Jahrgangsstufe*.
sichtfreigabe | String (Code) | 0..1 | Attribut gibt an, ob dieser Personenkontext aufgrund der Freigabe durch eine andere [Organisation](./organisation.md) sichtbar ist. Ist `sichtfreigabe` nicht gesetzt, so entspricht das dem Wert `nein`. Der Wert von Sichtfreigabe ist Boolean nach Codetabelle *Boolean*.
